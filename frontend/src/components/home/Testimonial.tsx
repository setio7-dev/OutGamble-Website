    ...
    <div className="w-full h-auto py-16 mt-12 flex flex-col justify-center gap-30" id="testimonial">
            ...
            <div className="flex items-center gap-8 justify-start py-2 duration-100 w-full" style={{ transform: `translateX(-${currentIndex}px)` }}>
              {data.map((item, index) => (
                <div className="flex shrink-0 items-center gap-8 justify-start" key={index}>
                  ...
                  <h1 className="text-black lg:text-[40px] text-[24px] font-poppins-bold">{item.text}</h1>
                </div>
              ))}
            </div>
          </div>
      ...
      {isMobile ? (
        <div className="w-full flex flex-col justify-center items-center">
          <AnimationDiv effect="fade-up" className="text-center flex flex-col gap-4 lg:w-200 w-full z-20">
            <h1 className="font-poppins-bold text-[24px] text-white">Cerita dari Para Pejuang <span className="text-primary">Lepas Judi</span></h1>
            <p className="font-poppins-regular lg:text-[14px] text-[12px] text-white">Butuh keberanian besar untuk mengakui, tekad kuat untuk berubah, dan dukungan dari orang-orang di sekitar. Namun setiap langkah kecil menjauh dari judi adalah kemenangan besar atas diri sendiri.</p>
          </AnimationDiv>
          <AnimationDiv effect="fade-up" delay={300} className="flex justify-center mt-10 items-center">
              <div className="bg-primary p-2 w-10 h-10 shrink-0 rounded-full flex justify-center items-center cursor-pointer translate-x-4 border-2 border-primary group hover:bg-white duration-500 hover:-rotate-6 z-2" onClick={handlePrevWithAnim}>
              ...
              <div className={`bg-white rounded-lg border-2 border-[#D8D8D8] flex flex-col justify-center items-center py-4 px-8 lg:w-160 w-full gap-4 transition-all duration-500 ${animateDirection === "next" ? "translate-x-10 opacity-0" : animateDirection === "prev" ? "-translate-x-10 opacity-0" : "opacity-100 translate-x-0"}`}>
                <img src={reoderedTestimonialData[3].img} className="rounded-full w-24 h-24" alt="" />
                <h1 className="font-poppins-semibold text-center text-[18px] text-primary">{reoderedTestimonialData[3].name}</h1>
                <p className="text-[10px] text-black font-poppins-regular text-justify">{reoderedTestimonialData[3].text}</p>
              </div>
              <div className="bg-primary p-2 w-10 h-10 shrink-0 rounded-full flex justify-center items-center cursor-pointer -translate-x-4 border-2 border-primary group hover:bg-white duration-500 hover:-rotate-6 z-2" onClick={handleNextWithAnim}>

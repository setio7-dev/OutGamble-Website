        ...
        <SafeAreaView classNameParent={'h-auto'} classNameChild={'flex flex-col justify-center'}>
            <div className='flex flex-col mt-18 items-center'>
                <AnimationDiv effect='fade-up' className="flex flex-col w-full justify-center items-center gap-2">
                    <h1 className='font-poppins-semibold text-thirdtinary lg:text-[35px] text-[24px]'>Komunitas Out Gamble</h1>
                    <p className='font-poppins-medium lg:text-[17px] text-[14px] text-center text-white lg:px-15 px-0 pt-2'>Temukan komunitas yang mendukung kesadaran digital dan membantu masyarakat menjauhi bahaya judi online. Bergabunglah untuk belajar, berbagi, dan tumbuh bersama dalam lingkungan positif!</p>
                </AnimationDiv>
                {communities.length > 0 ? (
                    <div className='grid lg:grid-cols-2 grid-cols-1 w-full mt-12 justify-between gap-x-10 lg:gap-y-5 gap-y-10'>
                        {communities.map((item, index) => (
                            <AnimationDiv effect='fade-up' delay={item.id * 200} key={index} className='flex flex-col bg-white rounded-[13px] pb-10 items-center'>
                                ...
                                <div className='flex flex-col items-center mt-4 lg:px-12 px-4'>
                                    <div className="flex flex-col items-center gap-2">
                                        <h1 className='font-poppins-bold text-center text-fourthtinary lg:text-[26px] text-[20px]'>{item.name}</h1>
                                        <p className='font-poppins-medium font-medium mb-4 lg:text-[14px] text-[12px] text-center text-gray-text'>{item.description}</p>
                                    </div>
